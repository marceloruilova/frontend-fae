...
import {Modal,ModalHeader,ModalBody,ModalFooter,Row,Col,FormGroup,Form,Input,Button} from 'reactstrap';
...
function DoctorModal(props){
    ...
    useEffect(()=>{
        const fetchDoctors=async()=>{
            ...
            setValue(data.data.length==0?"":`${data.data[0].ci}-${data.data[0].doctor_first_name}`)
        }
        fetchDoctors();
    },[])
    ...
    const onSubmit = (data) => {
        ...
        try {
          axios.post('http://localhost:3000/doctor', request).then((result) => {
            alert(result);
          });
        } catch (error) {
          alert(error);
      ...
      const submitPrescription = () => {
        ...
        const selectedDoctor=doctors.filter(doctor=>{return doctor.ci.includes(doctorci[0])})
        try {
            axios.post('http://localhost:3000/prescription/savedoctor',
    ...
    return (
        ...
        >
            <ModalHeader 
                toggle={props.toggle}
                >Agregar Médico Prescriptor</ModalHeader>
            <ModalBody><div><Row><Col xs="9">
                                <Autocomplete
                            ...
                            <Col><div onClick={toggle}>
                                ...
                                <ModalHeader 
                                    toggle={toggle}
                                    >Agregar Nuevo Doctor</ModalHeader>
