...
function Inventory() {
    ...
    const [value, setValue] = useState({name:"",presentation:"",concentration:"",stock:""});
    ...
    <div className="login-box-container">
      ...
      <tbody>
        ...
        <th scope="row">{nowuser.hce==undefined?"":nowuser.hce.id}</th>
          ...
          <td >
            {nowuser.prescription==undefined?"":nowuser.prescription.prescribing_doctor.doctor_first_name}
          </td>
          <td>
        ...
        <td>{nowuser.hce==undefined?"":nowuser.prescription.info_prescription.quantity}
          </td>
        <td>{nowuser.hce==undefined?"":nowuser.prescription.info_prescription.cie10.disease}
          </td>
            <td>{nowuser.hce==undefined?"":nowuser.prescription.info_prescription.price}
          </td>
          <td>{nowuser.hce==undefined?"":nowuser.prescription.info_prescription.ticket_number}
          </td>
        </tr>
      </tbody>
      </Table>
      ...
      <tbody>
        <tr >
          <td>
              <Autocomplete
               getItemValue={(item) =>(`${item.name} ${item.presentation} ${item.concentration}`)}
               items={inventorycopy}
               ...
               onChange={(e) => {
                ...
                let newArray=inventory.filter(aux=>{
                  return `${aux.name} ${aux.presentation} ${aux.concentration}`.includes(e.target.value);})
                setInventorycopy(newArray);setValue({name:valuearr[0],presentation:valuearr[1],concentration:valuearr[2]})}
              }
               onSelect={(val) => {
                let valuearr=val.split(" ");
                setValue({name:valuearr[0],presentation:valuearr[1],concentration:valuearr[2]})}
              }
              />
          </td>
          <td>
              <FormGroup>
                <Input
                  ...
                  name="presentation"
                  type="text"
                  defaultValue={value.presentation=""?"":value.presentation}
                  className="inputborder"
                  onChange={(e) =>
                    setValue({name:value.name,presentation: e.target.value,concentration:""})
                  }
                />
          </FormGroup>
          </td>
          ...
          <FormGroup>
                <Input
                  ...
                  name="concentration"
                  type="text"
                  defaultValue={value.concentration=""?"":value.concentration}
                  className="inputborder"
                  onChange={(e) =>
                    setValue({name:value.name,presentation: value.presentation,concentration:e.target.value})
                  }
                />
          </FormGroup>
          </td>
